...
class IP(dpkt.Packet):
	...
	def __calc_sum(self, upperlayer=False):
		...
		if (self.p == 6 or self.p == 17) and \
			...
			(isinstance(self.data, dpkt.Packet) and \
			self.data.sum == 0:
	...
	def unpack(self, buf):
		# IHL*4
		ol = ((buf[0] & 0xf) << 2) - self.__hdr_len__	# IHL - IP-len
		if ol < 0:
		...
		# TODO: test and parse separately
		if len(self._opts) > 0:
