...
class IP6(pypacker.Packet):
	...
	def _unpack(self, buf):
		...
		while next != 1234:
			...
			type = buf[off]
			len = 2 + buf[off + 1]*4
			opt = ext_hdrs_cls[next](buf[off:off+len])
			opts.append(opt)
			off += len
		...
		try:
			...
			type_instance = self._handler[ip.IP.__name__][type](buf[self.__hdr_len__:])
                        # set callback to calculate checksum
			type_instance.callback = self.callback_impl
			self._set_bodyhandler(type_instance)
		except Exception as ex:
			logger.debug(">>> IPv6: couldn't set handler: %d -> %s" % (type, ex))
			pass
