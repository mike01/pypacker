...
from pypacker.layer567 import diameter, dhcp, dns, hsrp, http, ntp, pmap, radius, rip, telnet, tpkt
# General testcases:
...
def get_pcap(fname, cnt=1000):
	...
	"""
	packet_bytes = []
	pcap = ppcap.Reader(path.join(path.dirname(path.dirname(__file__)), fname))

	for ts, buf in pcap:
		packet_bytes.append(bytearray(buf))
		cnt -= 1
...
class AllTestCase(unittest.TestCase):
	...
	def test_eth(self):
		...
		s = b"\x52\x54\x00\x12\x35\x02\x08\x00\x27\xa9\x93\x9e\x00\x00"
	...
	def test_concat(self):
		...
		print(p_all.bin())
		print(p_all2.bin())
		self.assertEqual(bytearray(tcp1.bin()), bytearray(tcp2.bin()))
		self.assertEqual(bytearray(p_all2.bin()), bytearray(p_all.bin()))
	...
	def test_reassemble(self):
		...
		with open(path.join(path.dirname(__file__), "certs_extracted_0.bin"), "rb") as fd:
		...
		with open(path.join(path.dirname(__file__), "certs_extracted_1.bin"), "rb") as fd:
			bts_assembled = fd.read()
		self.assertEqual(segments_ra, bts_assembled[:len(segments_ra)])
